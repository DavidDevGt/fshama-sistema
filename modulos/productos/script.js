...
busquedaProducto.on("keyup", debounce(function() {
    const consulta = $(this).val();
    cargarProductos(consulta);
}, 500)); // Esperamos 500ms después de que el usuario haya dejado de escribir para hacer la solicitud
    ...
    $.post('modulos/productos/ajax.php', data, (data) => {
        ...
        if (respuesta[0] === '1') {
            ...
            productos.forEach((producto) => {
                const descripcionTruncada = producto.descripcion.length > 10 ? producto.descripcion.substring(0, 8) + "..." : producto.descripcion;
                ...
                <tr>
                    ...
                    <td>
                        <div class="btn-group">
                            <button class="btn btn-success btn-sm" onclick="cargarEditarProducto(${producto.id_producto})">
            ...
            html = '<tr><td colspan="10">No hay productos que coincidan con la consulta.</td></tr>';
        }
...
const cargarEditarProducto = (idProducto) => {
    $.post('modulos/productos/ajax.php', { fnc: 'mostrar_producto', id_producto: idProducto }, (data) => {
        const respuesta = data.split("|");
        if (respuesta[0] === '1') {
...
const eliminarProducto = (idProducto) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.post('modulos/productos/ajax.php', {
...
const cargarCategorias = () => {
    $.post('modulos/productos/ajax.php', { fnc: 'mostrar_categorias' }, (data) => {
        const respuesta = data.split("|");
        let html = '';
...
const cargarUnidadesMedida = () => {
    $.post('modulos/productos/ajax.php', { fnc: 'mostrar_unidades_medida' }, (data) => {
        const respuesta = data.split("|");
